...
trait Generator {
  ...
  def getSupportedDialect(): Dialect
  ...
  def fieldLength(f: ClassFieldMetaData): Option[String] = MetaDataWithDialectExtractor.getAnnotationForDialect(Length, Some(getSupportedDialect()), f).map(_.getValue)

  def comment(a: HasAnnotations): Option[String] = MetaDataWithDialectExtractor.getAnnotationForDialect(Comment, Some(getSupportedDialect()), a).map(_.getValue)

  def notNull(f: ClassFieldMetaData): Boolean = MetaDataWithDialectExtractor.getAnnotationForDialect(NotNull, Some(getSupportedDialect()), f).nonEmpty
